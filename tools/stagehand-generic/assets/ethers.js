...
(() => {
  ...
  var NETWORK_CONFIGS = {
    ...
    84531: { name: "Base Sepolia", rpcUrl: "https://sepolia.base.org" },
    11155111: { name: "Sepolia", rpcUrl: "https://eth-sepolia.g.alchemy.com/v2/demo" },
    5: { name: "Goerli", rpcUrl: "https://eth-goerli.g.alchemy.com/v2/demo" },
    31337: { name: "Anvil Local", rpcUrl: "http://127.0.0.1:8545" }
  };
  ...
  var EthersProvider = class {
    constructor(networkId, privateKey, windowProvider) {
      ...
      // For filters, we store them in-memory. Real wallets wouldn't do much with these,
      // but some dApps call them. We can stub them out.
      this.filterIndex = 1;
      this.filters = {};
      ...
      this.provider = new ethers_exports.JsonRpcProvider(network.rpcUrl);
      this.signer = new ethers_exports.Wallet(privateKey, this.provider);
      this.chainId = `0x${networkId.toString(16)}`;
      this.networkVersion = networkId.toString();
      this.selectedAddress = this.signer.address;
      this._state.accounts = [this.selectedAddress];
      this._state.initialized = true;
      this._windowProvider = windowProvider;
      console.log("Hanzo Ethers>> Initialized with network:", network.name);
    }
    // Event handling with window.ethereum sync
    updateWindowProvider() {
      if (this._windowProvider) {
        this._windowProvider.selectedAddress = this.selectedAddress;
        this._windowProvider.chainId = this.chainId;
        this._windowProvider.networkVersion = this.networkVersion;
        this._windowProvider.isConnected = this._state.isConnected;
      }
    }
    emit(eventName, ...args) {
      const listeners = this._events[eventName] || [];
      listeners.forEach((listener) => listener(...args));
      if (eventName === "accountsChanged" && args[0]?.[0]) {
        this.selectedAddress = args[0][0];
        this.updateWindowProvider();
      } else if (eventName === "chainChanged") {
        this.chainId = args[0];
        this.networkVersion = parseInt(this.chainId, 16).toString();
        this.updateWindowProvider();
      } else if (eventName === "connect" || eventName === "disconnect") {
        this._state.isConnected = eventName === "connect";
        this.updateWindowProvider();
      }
    }
    // Event handling
    ...
    async request({ method, params = [] }) {
      console.log("Hanzo Ethers>> request", method, params);
      switch (method) {
        case "eth_chainId":
          return this.chainId;
        case "eth_accounts":
        ...
        case "eth_requestAccounts":
          return this.enable();
        // Often-called older version of chain ID
        case "net_version":
        ...
        case "eth_sign": {
          const [signAddress, signMessage] = params;
          if (signAddress.toLowerCase() !== this.selectedAddress.toLowerCase()) {
            throw new ProviderRpcError(4100, "Requested account not selected");
          }
          return this.signer.signMessage(ethers_exports.getBytes(signMessage));
        }
        case "personal_sign": {
        ...
        case "eth_signTypedData_v4": {
          ...
          return this.signTypedData(signerAddress, JSON.parse(typedData));
        }
        case "eth_call": {
          ...
          return this.provider.call({
            ...
            value: callParams.value ? ethers_exports.getBigInt(callParams.value) : void 0,
            gasLimit: callParams.gas ? ethers_exports.getBigInt(callParams.gas) : void 0
          });
        }
        case "eth_estimateGas": {
          const [estimateParams] = params;
          return this.provider.estimateGas(estimateParams);
        }
        case "eth_gasPrice": {
          const feeData = await this.provider.getFeeData();
          return feeData.gasPrice?.toString() || "0x0";
        }
        // Commonly used read methods
        case "eth_blockNumber": {
          const blockNumber = await this.provider.getBlockNumber();
          return ethers_exports.toQuantity(blockNumber);
        }
        case "eth_getBalance": {
          const [balanceAddress, blockTag = "latest"] = params;
          const balance = await this.provider.getBalance(balanceAddress, blockTag);
          return ethers_exports.toQuantity(balance);
        }
        case "eth_getTransactionCount": {
          const [account, blockTag = "latest"] = params;
          const txCount = await this.provider.getTransactionCount(account, blockTag);
          return ethers_exports.toQuantity(txCount);
        }
        case "eth_getTransactionByHash": {
          ...
          return {
            ...
            blockNumber: tx.blockNumber ? ethers_exports.toQuantity(tx.blockNumber) : null,
            from: tx.from,
            to: tx.to,
            data: tx.data,
            gas: tx.gasLimit ? ethers_exports.toQuantity(tx.gasLimit) : "0x0",
            gasPrice: tx.gasPrice ? ethers_exports.toQuantity(tx.gasPrice) : "0x0",
            value: tx.value ? ethers_exports.toQuantity(tx.value) : "0x0",
            nonce: tx.nonce ? ethers_exports.toQuantity(tx.nonce) : "0x0",
            v: tx.signature?.v ? ethers_exports.toQuantity(tx.signature.v) : "0x0",
            r: tx.signature?.r ?? "0x",
            s: tx.signature?.s ?? "0x"
          };
        }
        case "eth_getTransactionReceipt": {
          ...
          const receipt = await this.provider.getTransactionReceipt(receiptTxHash);
          if (!receipt) return null;
          return {
            blockHash: receipt.blockHash,
            blockNumber: ethers_exports.toQuantity(receipt.blockNumber),
            contractAddress: receipt.contractAddress,
            cumulativeGasUsed: ethers_exports.toQuantity(receipt.cumulativeGasUsed),
            effectiveGasPrice: ethers_exports.toQuantity(receipt.gasPrice ?? 0),
            from: receipt.from,
            gasUsed: ethers_exports.toQuantity(receipt.gasUsed),
            logs: receipt.logs.map((log) => ({
              address: log.address,
              blockHash: log.blockHash,
              blockNumber: ethers_exports.toQuantity(log.blockNumber),
              data: log.data,
              logIndex: ethers_exports.toQuantity(log.index),
              removed: false,
              topics: log.topics,
              transactionHash: log.transactionHash,
              transactionIndex: ethers_exports.toQuantity(log.transactionIndex)
            })),
            logsBloom: receipt.logsBloom,
            status: ethers_exports.toQuantity(receipt.status ?? 0),
            to: receipt.to,
            transactionHash: receipt.hash,
            transactionIndex: ethers_exports.toQuantity(receipt.index),
            type: ethers_exports.toQuantity(receipt.type ?? 0)
          };
        }
        case "eth_getBlockByNumber": {
          const [blockNumberOrTag, includeTransactions] = params;
          const block = await this.provider.getBlock(blockNumberOrTag, includeTransactions);
          if (!block) return null;
          return {
            number: ethers_exports.toQuantity(block.number),
            hash: block.hash,
            parentHash: block.parentHash,
            nonce: block.nonce,
            difficulty: ethers_exports.toQuantity(block.difficulty),
            gasLimit: ethers_exports.toQuantity(block.gasLimit),
            gasUsed: ethers_exports.toQuantity(block.gasUsed),
            miner: block.miner,
            extraData: block.extraData,
            stateRoot: block.stateRoot,
            receiptsRoot: block.receiptsRoot,
            baseFeePerGas: block.baseFeePerGas ? ethers_exports.toQuantity(block.baseFeePerGas) : null,
            timestamp: ethers_exports.toQuantity(block.timestamp),
            transactions: includeTransactions ? block.prefetchedTransactions.map((tx) => ({
              blockHash: tx.blockHash,
              blockNumber: tx.blockNumber ? ethers_exports.toQuantity(tx.blockNumber) : null,
              from: tx.from,
              gas: ethers_exports.toQuantity(tx.gasLimit),
              gasPrice: ethers_exports.toQuantity(tx.gasPrice || 0),
              hash: tx.hash,
              input: tx.data || "0x",
              nonce: ethers_exports.toQuantity(tx.nonce),
              to: tx.to,
              transactionIndex: ethers_exports.toQuantity(tx.index),
              value: ethers_exports.toQuantity(tx.value),
              type: ethers_exports.toQuantity(tx.type || 0),
              chainId: ethers_exports.toQuantity(tx.chainId),
              v: tx.signature?.v ? ethers_exports.toQuantity(tx.signature.v) : "0x0",
              r: tx.signature?.r || "0x0",
              s: tx.signature?.s || "0x0"
            })) : block.transactions,
            uncles: []
          };
        }
        // Filter endpoints
        // (Minimal stubs that let dApps proceed without error. Real wallets rarely do on-chain filters.)
        case "eth_newFilter": {
          const [filterOptions] = params;
          const filterId = `0x${(this.filterIndex++).toString(16)}`;
          this.filters[filterId] = { type: "log", options: filterOptions };
          return filterId;
        }
        case "eth_newBlockFilter": {
          const filterId = `0x${(this.filterIndex++).toString(16)}`;
          this.filters[filterId] = { type: "block", options: {} };
          return filterId;
        }
        case "eth_newPendingTransactionFilter": {
          const filterId = `0x${(this.filterIndex++).toString(16)}`;
          this.filters[filterId] = { type: "pending", options: {} };
          return filterId;
        }
        ...
        case "eth_getFilterChanges": {
          const [filterId] = params;
          if (!this.filters[filterId]) return [];
          const filter = this.filters[filterId];
          if (filter.type === "log") return [];
          if (filter.type === "block") return [];
        ...
        // EIP-747: some dApps use `wallet_watchAsset` to add tokens
        case "wallet_watchAsset": {
          const options = params[0] || {};
          console.log("wallet_watchAsset called with", options);
          return true;
        }
        ...
        case "wallet_addEthereumChain":
          ...
          return this.addChain(chainParams);
        default:
    ...
    async enable() {
      try {
        ...
        console.log("Hanzo Ethers>> Connected to chain:", chainId);
        const accounts = await this.request({ method: "eth_requestAccounts", params: [] });
        console.log("Hanzo Ethers>> Connected with accounts:", accounts);
    ...
    async sendTransaction(txParams) {
      const tx = await this.signer.sendTransaction({
        to: txParams.to,
        data: txParams.data,
        value: txParams.value ? ethers_exports.getBigInt(txParams.value) : 0n,
        gasLimit: txParams.gas ? ethers_exports.getBigInt(txParams.gas) : void 0,
        maxFeePerGas: txParams.maxFeePerGas ? ethers_exports.getBigInt(txParams.maxFeePerGas) : void 0,
        maxPriorityFeePerGas: txParams.maxPriorityFeePerGas ? ethers_exports.getBigInt(txParams.maxPriorityFeePerGas) : void 0,
        gasPrice: txParams.gasPrice ? ethers_exports.getBigInt(txParams.gasPrice) : void 0
      });
      return tx.hash;
    }
    // Signing methods
    async personalSign(message, address) {
      if (address.toLowerCase() !== this.selectedAddress.toLowerCase()) {
        throw new ProviderRpcError(4100, "Requested account not selected");
      }
      const messageBytes = ethers_exports.getBytes(message);
    ...
    async addChain(params) {
      const networkId = parseInt(params.chainId, 16);
      NETWORK_CONFIGS[networkId] = {
        name: params.chainName,
        rpcUrl: params.rpcUrls[0]
      };
      return this.switchChain(params.chainId);
    }
    ...
    async connect() {
      try {
        ...
        if (this.status === "connecting") {
          return new Promise((resolve) => {
            this.once("connect", () => resolve([this.selectedAddress]));
          });
        }
    ...
    disconnect() {
      if (this.status === "connected") {
        ...
        this.emit("disconnect", { code: 1e3, message: "Disconnected" });
  ...
  var ethersProviderInfo = {
    uuid: "354c460f-7aa1-4bbe-a090-4d9f0a6b1d8a",
    name: "MetaMask",
    icon: "data:image/svg+xml;base64,",
    rdns: "io.metamask"
  };
  function initializeEthersProvider(sk, networkId = 1) {
    ...
    const provider = new EthersProvider(networkId, sk, windowProvider);
    window.ethereum = {
      ...windowProvider,
      // Standard methods
      ...
      sendAsync: (payload, callback) => {
        ...
        provider.request({ method: payload.method, params: payload.params }).then((result) => callback(null, { id: payload.id, jsonrpc: "2.0", result })).catch((error) => callback(error, null));
      },
    ...
    provider._windowProvider = window.ethereum;
    provider.updateWindowProvider();
    const announceProvider = () => {
      const announceEvent = new CustomEvent("eip6963:announceProvider", {
        bubbles: true,
        cancelable: false,
        detail: {
          info: ethersProviderInfo,
    ...
    console.log("Hanzo Ethers>> Provider initialized and ready (not auto-connected)");
    return provider;
  }
  if (typeof window !== "undefined") {
    window.initEthersProvider = initializeEthersProvider;
  }
})();
