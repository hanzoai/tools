...
export const run: Run<Configurations, Parameters, Result> = async (
  ...
  try {
    ...
    console.warn("error while waiting for page to load:", error);
  }
  ...
  const safeFileName = parameters.url
    .replace(/\./g, "_")
    .replace(/[^a-zA-Z0-9_-]/g, "");
  const safeDate = new Date().toISOString().replace(/[:.]/g, "-");
  Deno.writeTextFileSync(
    (await getHomePath()) + `/raw.${safeFileName}.${safeDate}.html`,
    html
  );
